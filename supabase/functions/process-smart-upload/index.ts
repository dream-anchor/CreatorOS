...
serve(async (req) => {
  ...
  try {
    ...
    const { files, rawText } = await req.json();

    if (!files || !Array.isArray(files) || files.length === 0) {
      throw new Error("Keine Bilder hochgeladen");
    }

    console.log(`[smart-upload] Processing ${files.length} files for user ${user.id}`);

    // Step 1: Determine format
    const format = files.length > 1 ? "carousel" : "single";
    console.log(`[smart-upload] Format detected: ${format}`);
    ...
    for (let i = 0; i < files.length; i++) {
      ...
      if (uploadError) {
        console.error(`[smart-upload] Upload error for file ${i}:`, uploadError);
      ...
      console.log(`[smart-upload] Uploaded file ${i + 1}/${files.length}: ${fileName}`);
    }
    ...
    console.log(`[smart-upload] Generating caption with AI...`);
...
Antworte NUR mit der fertigen Caption + Hashtags.`;
    ...
    // Add first image for vision analysis (limit to first to avoid token limits)
    userContent.push({
      type: "image_url",
    ...
    if (!aiResponse.ok) {
      ...
      console.error("[smart-upload] AI error:", errorText);
    ...
    console.log(`[smart-upload] Generated caption: ${generatedCaption.substring(0, 100)}...`);

    // Step 5: Find next free slot (Gap-Filler Algorithm)
    const scheduledAt = await findNextFreeSlot(supabase, user.id);
    const scheduledDay = getDayName(scheduledAt);
    const scheduledDateFormatted = formatDate(scheduledAt);

    console.log(`[smart-upload] Scheduling for: ${scheduledDay}, ${scheduledDateFormatted} at 18:00`);
    ...
    if (postError) {
      console.error("[smart-upload] Post creation error:", postError);
      throw new Error(`Post konnte nicht erstellt werden: ${postError.message}`);
    }

    console.log(`[smart-upload] Post created: ${post.id}`);
    ...
    if (format === "carousel") {
      ...
      console.log(`[smart-upload] Created ${uploadedUrls.length} slide assets`);
    }
